...
export function Header({ onMobileMenuToggle }: { onMobileMenuToggle?: () => void }) {
  const { settings, stats, cart } = useApp();
  ...
  const handleBackup = async () => {
    ...
    setIsBackingUp(true);
    try {
      // Pull everything from the server, in parallel
      const [
        products,
        categories,
        customers,
      ...
      a.href = url;
      a.download = `${(settings.storeName || "store").replace(/\s+/g, "-").toLowerCase()}-backup-${dateStamp}.json`;
      ...
      <div className="flex items-center justify-between px-4 lg:px-6 h-16">
        ...
        <div className="flex items-center space-x-2 sm:space-x-3 shrink-0">
          ...
          <button
            ...
            className="p-2 text-slate-600 hover:text-slate-900 hover:bg-slate-100 rounded-lg transition-colors border border-slate-200/80 shrink-0 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
          >
